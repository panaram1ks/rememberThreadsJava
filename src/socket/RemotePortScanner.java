...
public class RemotePortScanner {
    ...
    public static void main(String[] args) {
        ...
        //as long as isValid is false, try to obtain a valid port numbers
        while (!isValid) {
            try {
            ...
            } catch (Exception e3) {// error when trying to read user input
                System.out.println("Cannot read the last port number: " + e3.toString());
            }
        }
        ...
        while (port >= fromPort && port <= toPort) { //use the given port range
            ...
            port++;//increment port in order to test the next port
        }
